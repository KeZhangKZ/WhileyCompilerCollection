...
public interface Build {
	...
	public interface Graph {
		/**
		 * Determine the entry (the parent) that a given entry (the child) is derived
		 * from, or null if no such entry exists. An entry is said to derive from
		 * another if it generated from the other during the compilation process. For
		 * example, a source file is compiled into a binary or intermediate file. The
		 * latter is said to be derived from the former.
		 *
		 * @param child
		 *            The child entry which is derived from zero or more parent entries.
		 * @return
		 */
	...
	public interface Platform extends Feature {
		 ...
		 *
		 * @param configuration
		 *            Configuration options matching the platform's schema
		 */
